...
if which nginx >/dev/null 2>&1; then
	stderr="$(nginx -V </dev/stdin 2>&1 1>&3)"
	echo $stderr |sed "s/.*nginx\/\([0-9\.]*\).*/nginx (\1)/g"
fi
if which python >/dev/null 2>&1; then
	stderr="$(python --version </dev/stdin 2>&1 1>&3)"
	echo $stderr |awk '{printf("%s (%s)\n",$1,$2);}'
fi

if which python2.7 >/dev/null 2>&1; then
	stderr="$(python2.7 --version </dev/stdin 2>&1 1>&3)"
	echo $stderr |awk '{printf("%s (%s)\n",$1,$2);}'
fi
...
IFS='
'
stderr="$(tcpdump --version </dev/stdin 2>&1 1>&3)"
count=0
for item in $stderr
do
	if [ $count = 2 ]; then
