_NAMESPACE_=httpdtest
kubectl create namespace $_NAMESPACE_
kubectl apply -f http_backend.yaml -n $_NAMESPACE_
sleep 3
_COUNTER_=0
while [ `kubectl get pod -n $_NAMESPACE_| grep -m 1 httpd-deployment | awk '{print $3}' ` != "Running" ] ; do
       _COUNTER_=$((( _COUNTER_ + 1 )))
       echo  NOTICE: it is infinity loop here now ...
       echo http server pod is not ready. COUNTER = $_COUNTER_. idle 1 more second ...
       sleep 1
done
_SVCIP_=`kubectl get svc -n $_NAMESPACE_| grep -m 1 httpdtest-service | awk '{print $3}' `
_SVCPORT_=`kubectl get svc -n $_NAMESPACE_| grep -m 1 httpdtest-service | awk '{print $5}' | cut -d '/' -f 1`
echo $_SVCIP_:$_SVCPORT_ 
sleep 1
kubectl apply -f waf_traversal.yml 
sleep 1
kubectl apply -f waf_injection.yml
sleep 1
kubectl apply -f waf_cross.yml
sleep 1
kubectl apply -f waf_execution.yml
sleep 1
kubectl apply -f waf_group_httpd.yml
sleep 10
