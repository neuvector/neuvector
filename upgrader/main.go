	...
	cmd := &cli.Command{
		Flags: []cli.Flag{
			...
			&cli.StringFlag{
				...
				Sources: cli.EnvVars("INTERNAL_SECRET_NAME"),
			},
			&cli.DurationFlag{
				Name:    "timeout",
			...
			},
			{
				...
				Flags: []cli.Flag{
					&cli.DurationFlag{
						...
						Sources: cli.EnvVars("ROLLOUT_TIMEOUT"),
					},
					&cli.IntFlag{
						Name:    "rsa-key-length",
						Value:   4096,
						Usage:   "RSA key length when creating new internal key and certificate",
						Sources: cli.EnvVars("RSA_KEY_LENGTH"),
					},
					&cli.DurationFlag{
