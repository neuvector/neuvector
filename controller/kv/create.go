...
func CreateFedDefDlpRules(withlock bool) {
	...
	clusHelper.PutDlpSensor(dlpsensor, false)
}

func CreateFedDefDlpRuleSensor() {
	if fedRole, _ := getFedRole(); fedRole == api.FedRoleJoint || fedRole == api.FedRoleMaster {
		CreateFedDefDlpRules(false)
		CreateFedPreDlpSensor(false)
	}
}
...
func CreatePreFedWafSensor(withlock bool) {
	if !withlock {
		clusterLockWait := time.Duration(time.Second * 4)
		lock, err := clusHelper.AcquireLock(share.CLUSLockPolicyKey, clusterLockWait)
		if err != nil {
...
func CreateDefFedWafRules(withlock bool) {
	...
	clusHelper.PutWafSensor(wafsensor, false)
}

func CreateDefFedWafRuleSensor() {
	if fedRole, _ := getFedRole(); fedRole == api.FedRoleJoint || fedRole == api.FedRoleMaster {
		CreateDefFedWafRules(false)
		CreatePreFedWafSensor(false)
	}
}
