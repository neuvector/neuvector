...
func ScanRunning(scanner string, agentID, id string, objType share.ScanObjectType, timeout time.Duration) (*share.ScanResult, error) {
	...
	if err := clusHelper.PutScannerStats(scanner, objType, result); err != nil {
		log.WithFields(log.Fields{"error": err}).Error("PutScannerStats")
	}
...
func ScanImage(scanner string, ctx context.Context, req *share.ScanImageRequest) (*share.ScanResult, error) {
	...
	clusHelper := kv.GetClusterHelper()
	if err := clusHelper.PutScannerStats(scanner, share.ScanObjectType_IMAGE, result); err != nil {
		log.WithFields(log.Fields{"error": err}).Error("PutScannerStats")
	}
...
func ScanPackage(ctx context.Context, pkgs []*share.ScanAppPackage) (*share.ScanResult, error) {
	...
	clusHelper := kv.GetClusterHelper()
	if err := clusHelper.PutScannerStats(scanner, share.ScanObjectType_SERVERLESS, result); err != nil {
		log.WithFields(log.Fields{"err": err}).Error("PutScannerStats")
	}
