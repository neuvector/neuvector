...
func replaceFedDlpSensorGroups(DlpSensors []*share.CLUSDlpSensor, DlpGroups []*share.CLUSDlpGroup) bool {
	...
	* created in upgrading process, create it here.
	...
	exSensorMap := make(map[string]*share.CLUSDlpSensor)
	...
	for _, esen := range existingSensors {
		...
		if esen != nil && esen.CfgType == share.FederalCfg {
			if _, ok := dlpSenMap[esen.Name]; !ok { // in existing but not in new. so delete it
	...
	for _, dsen := range dlpSenMap {
		if dsen != nil {
			...
			_, found := exSensorMap[dsen.Name]
			if !found || (found && !reflect.DeepEqual(*dsen, *exSensorMap[dsen.Name])) {
				clusHelper.PutDlpSensorTxn(txn, dsen)
			}
		}
	}
	clusHelper.PutDlpSensorTxn(txn, defsensor)

	exDlpgrpMap := make(map[string]*share.CLUSDlpGroup)
	existingDlpgrps := clusHelper.GetAllDlpGroups()
	for _, edlpgrp := range existingDlpgrps {
		if edlpgrp != nil {
			exDlpgrpMap[edlpgrp.Name] = edlpgrp
		}
		if edlpgrp != nil && edlpgrp.CfgType == share.FederalCfg {
			if _, ok := dlpGrpMap[edlpgrp.Name]; !ok { // in existing but not in new. so delete it
				clusHelper.DeleteDlpGroup(txn, edlpgrp.Name)
			}
		}
	}
	...
	for _, dlpgrp := range dlpGrpMap {
		if dlpgrp != nil {
			_, found := exDlpgrpMap[dlpgrp.Name]
			if !found || (found && !reflect.DeepEqual(*dlpgrp, *exDlpgrpMap[dlpgrp.Name])) {
				clusHelper.PutDlpGroupTxn(txn, dlpgrp)
			}
		}
	}
...
func PromoteFedDlpGroup(grpName string, acc *access.AccessControl, login *loginSession) {
	...
	if cached, _ := cacher.GetDlpGroup(grpName, acc); cached == nil || cached.CfgType == api.CfgTypeGround {
		log.WithFields(log.Fields{"group": grpName}).Debug("Local dlp group does not exist.")
		return
	} else {
